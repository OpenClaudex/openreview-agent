...
- 脚本启动时会自动读取 `~/.openreview.env`，所以常用命令无需手动 `source`；也可以用命令行参数临时覆盖。
- 若在交互式终端运行且只设置了用户名，脚本会提示输入密码；密码使用 `getpass`，不会回显。
...
脚本位置：`~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py`
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py inspect \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py profile-match \
  --name "Rang Li" \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py fetch \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py plan \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py dry-run \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py submit \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py run \
  --source https://openreview.net/forum?id=ABC123 \
  --target-venue NeurIPS.cc/2026/Conference \
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_transfer.py transfer \
  --source https://openreview.net/forum?id=ABC123 \
  --target-venue NeurIPS.cc/2026/Conference \
  --work-dir /tmp/or_transfer/
```
...
```
python3 ~/.codebuddy/skills/openreview-agent/scripts/or_batch.py \
